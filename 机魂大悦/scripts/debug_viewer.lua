...
--- 将节点树的所有模型零件变成独立物理碎片向外飞散
local function ExplodeModelNode(scene, rootNode, centerPos, explosionForce)
    ...
    for _, p in ipairs(parts) do
        local debris = scene:CreateChild("DebugExplodePart")
        ...
        table.insert(debrisNodes, debris)
    end

    print(string.format("[DebugViewer] Exploded %d parts", #parts))
    return debrisNodes
end

-- 死亡效果相关状态
local explodeDebris_ = {}       -- 飞散碎片节点列表
local explodeTimer_ = -1        -- 倒计时重建（<0 表示未激活）
local EXPLODE_REBUILD_DELAY = 3.0  -- 飞散后重建延迟（秒）
...
local function LoadModel(modelIndex)
    ...
    elseif modelDef.procedural == "boss" then
        -- 程序化 BOSS 战车
        isProcedural_ = true
        local modelNode, joints = BossBuilder.Build(currentModelNode_)
        -- BOSS 高度约 5.3m（底盘1.3+上层结构0.6+炮塔3.3），长约 6m
        camDistance_ = 14.0
        camTarget_ = Vector3(0, 2.6, 0)
        print("[DebugViewer] Loaded procedural BOSS model")

    elseif modelDef.procedural == "boss2" then
        -- 程序化 BOSS2 四轴无人机
        isProcedural_ = true
        local modelNode, result = Boss2Builder.Build(currentModelNode_)
        -- 无人机翼展~10.5m (1.5x), 高~4.2m, 机身中心在 y=3.0 (1.5x)
        camDistance_ = 15.0
        camTarget_ = Vector3(0, 3.0, 0)
        print("[DebugViewer] Loaded procedural BOSS2 drone model")

    elseif modelDef.procedural == "boss_combined" then
        -- 合体: 无人机底座 + 战车搭载在顶部
        isProcedural_ = true
        local droneNode, droneResult = Boss2Builder.Build(currentModelNode_)
        -- 在无人机挂载点上构建战车, 反向补偿父级缩放使战车保持原始尺寸
        local bossNode = droneResult.cargoMountPoint:CreateChild("BossTank")
        local parentScale = modelDef.scale or 1.0
        bossNode.scale = Vector3.ONE * (1.0 / parentScale)
        local tankModelNode, tankResult = BossBuilder.Build(bossNode)
        -- 合体总高度: 无人机机身y=2.0 + 平台0.38 + 战车高度~5.3m ≈ 7.7m
        camDistance_ = 18.0
        camTarget_ = Vector3(0, 4.0, 0)
        print("[DebugViewer] Loaded combined BOSS (Drone + Tank)")
    ...
    -- 统计模型零件数（StaticModel 组件数量）
    local partsList = {}
    CollectParts(currentModelNode_, partsList)
    local partsCount = #partsList

    -- 更新信息标签
    if modelInfoLabel_ then
        modelInfoLabel_.text = "模型: " .. modelDef.name .. "\n零件数: " .. tostring(partsCount)
    end

    print("[DebugViewer] Model loaded: " .. modelDef.name .. " (" .. partsCount .. " parts)")
end
    ...
    local leftPanel = UI.Panel {
        ...
        children = {
            ...
            UI.Button {
                ...
                width = "100%",
                height = 36,
                backgroundColor = { 150, 60, 20, 220 },
                textColor = { 255, 220, 180, 255 },
                hoverBackgroundColor = { 180, 80, 30, 240 },
                pressedBackgroundColor = { 120, 50, 15, 240 },
                borderRadius = 6,
                onClick = function(self)
                    if not currentModelNode_ or explodeTimer_ >= 0 then return end
                    local modelDef = CONFIG.Models[currentModelIndex_]
                    local isBossType = modelDef.procedural == "boss"
                        or modelDef.procedural == "boss2"
                        or modelDef.procedural == "boss_combined"
                    if not isBossType then
                        print("[DebugViewer] 死亡效果仅支持 BOSS 模型")
                        return
                    end
                    -- 收集零件并飞散
                    local center = currentModelNode_.worldPosition
                    -- boss2/boss_combined 中心偏高
                    if modelDef.procedural == "boss2" or modelDef.procedural == "boss_combined" then
                        center = center + Vector3(0, 3.0, 0)
                    elseif modelDef.procedural == "boss" then
                        center = center + Vector3(0, 2.0, 0)
                    end
                    explodeDebris_ = ExplodeModelNode(scene_, currentModelNode_, center, 22)
                    -- 隐藏原模型（移除所有子节点内容）
                    currentModelNode_:RemoveAllChildren()
                    -- 启动重建倒计时
                    explodeTimer_ = EXPLODE_REBUILD_DELAY
                    print("[DebugViewer] Death effect triggered, rebuilding in " .. EXPLODE_REBUILD_DELAY .. "s")
                end,
            },
...
local function HandleDebugUpdate(eventType, eventData)
    ...
    -- 死亡效果重建倒计时
    if explodeTimer_ >= 0 then
        explodeTimer_ = explodeTimer_ - dt
        if explodeTimer_ < 0 then
            -- 清理飞散碎片
            for _, node in ipairs(explodeDebris_) do
                if node then node:Remove() end
            end
            explodeDebris_ = {}
            -- 重建当前模型
            LoadModel(currentModelIndex_)
            print("[DebugViewer] Model rebuilt after death effect")
        end
    end
    ...
    if wheel ~= 0 and not UI.IsPointerOverUI() then
        camDistance_ = Clamp(camDistance_ - wheel * 0.005, 1.0, 40.0)
    end
