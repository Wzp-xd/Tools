...
-- ---------------------------------------------------------------
-- 🔧 临时开关：退出广告即奖励（无需看完）
-- 背景：当前广告 SDK 存在 BUG，部分机型广告未看完也不回调 success，
--       导致用户正常观看却无法获得奖励。临时改为只要进入广告再退出就发放。
-- true  = 玩家进入广告后退出/看完，无论 success 与否都发放奖励（当前临时方案）
-- false = 必须看完广告(success=true)才发放奖励（广告BUG修复后改回 false）
-- 影响范围：所有通过 AdDialog.Show() 触发的广告奖励（撤回、续命、道具等）
-- ---------------------------------------------------------------
local GRANT_ON_AD_RETURN = true
...
function _startAd()
    ...
    sdk:ShowRewardVideoAd(function(result)
        ...
        if result.success then
            -- 广告完整观看，发放奖励
            _onAdFinished()
        elseif GRANT_ON_AD_RETURN then
            -- 临时方案：玩家退出广告回到游戏，无论是否看完都发放奖励
            _onAdFinished()
        else
            -- 正常流程：广告未看完，不发放奖励
