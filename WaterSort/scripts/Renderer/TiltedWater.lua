...
--- 获取颜色（支持迷雾层: 负数 → 迷雾色）
---@param colors table[] 主题颜色表
---@param colorIdx integer
---@return table|nil color {r,g,b}
local function getSegColor(colors, colorIdx)
    if colorIdx < 0 then
        return Config.tube.fog.color
    end
...
--- 判断一个点是否在某条水平线上
local function isOnEdgeY(py, edgeY, tolerance)
    return math.abs(py - edgeY) < tolerance
end

--- 获取点所在的弧线参数（支持多条水平弧线）
--- @return number|nil edgeY, number|nil arcRY
local function matchArcEdge(py, arcs, tolerance)
    for _, arc in ipairs(arcs) do
        if isOnEdgeY(py, arc.edgeY, tolerance) then
            return arc.edgeY, arc.arcRY
        end
    end
    return nil, nil
end
...
--- 支持多条弧线边（顶边和底边各自独立的弧线）
--- @param vg userdata
--- @param band table[] 多边形顶点
--- @param arcs table[] 弧线配置列表 { {edgeY=number, arcRY=number}, ... }
local function drawBandWithArcs(vg, band, arcs)
    local tolerance = 0.5
    ...
    for k = 2, #band do
        ...
        local cur = band[k]
        -- 检查两个点是否在同一条弧线边上
        local prevEdgeY, _ = matchArcEdge(prev.y, arcs, tolerance)
        local curEdgeY, curArcRY = matchArcEdge(cur.y, arcs, tolerance)
        if prevEdgeY and curEdgeY and prevEdgeY == curEdgeY then
            local edgeY = curEdgeY
            local arcRY = curArcRY
    ...
    local lastEdgeY, _ = matchArcEdge(lastPt.y, arcs, tolerance)
    local firstEdgeY, firstArcRY = matchArcEdge(firstPt.y, arcs, tolerance)
    if lastEdgeY and firstEdgeY and lastEdgeY == firstEdgeY then
        local edgeY = firstEdgeY
        local arcRY = firstArcRY
...
local function drawBandWithArc(vg, band, edgeY, arcRY)
    drawBandWithArcs(vg, band, { { edgeY = edgeY, arcRY = arcRY } })
end
...
function TiltedWater.draw(nvg, cx, cy, pivotWX, pivotWY, angle, origLayers, removedCount)
    ...
    if absAngle >= math.pi * 0.5 then
        ...
        -- 极端角度只用底层颜色
        local color = getSegColor(colors, segments[1].colorIdx)
    ...
    for i = 1, #segments do
        local seg = segments[i]
        local color = getSegColor(colors, seg.colorIdx)
        ...
        if #band >= 3 then
            ...
            -- 顶边弧线
            if isTop then
                arcs[#arcs + 1] = { edgeY = bounds.top, arcRY = -surfaceArcRY }
            else
                arcs[#arcs + 1] = { edgeY = bounds.top, arcRY = boundaryArcRY }
            end

            -- 底边弧线（非最底层 segment 的底边也需要向下弧线）
            if not isBottom then
                arcs[#arcs + 1] = { edgeY = bounds.bottom, arcRY = boundaryArcRY }
            end

            nvgBeginPath(vg)
            drawBandWithArcs(vg, band, arcs)
            nvgClosePath(vg)
    ...
    for i = 2, #segments do
        ...
        local belowColor = getSegColor(colors, below.colorIdx)
    ...
    if surfLeftX and surfRightX then
        ...
        if surfWidth > 4 then
            ...
            local topColor = getSegColor(colors, topSeg.colorIdx)
            if topColor then
