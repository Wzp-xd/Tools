...
local function hitTestTube(localX, localY, canvasW, canvasH)
    ...
    for i = 1, game.tubeCount do
        ...
        local top = pos.y - tube.height / 2 - Config.interaction.selectOffset - padY
        local bottom = pos.y + tube.height / 2 + padY
...
function GameCanvas:Render(nvg)
    ...
    -- 阴影层
    for i = 1, game.tubeCount do
        if not (pour.phase ~= "none" and i == pour.srcIdx) then
            local pos = positions[i]
            local cx = l.x + pos.x
            local cy = l.y + pos.y + (anim.tubeOffsetY[i] or 0)
            nvgBeginPath(nvg)
            nvgEllipse(nvg, cx + render.shadowOffsetX, cy + tube.height / 2 + render.shadowOffsetY,
                tube.width / 2 + render.shadowRadiusX, render.shadowRadiusY)
            nvgFillColor(nvg, nvgRGBA(0, 0, 0, render.shadowAlpha))
            nvgFill(nvg)
        end
    end
    ...
    for i = 1, game.tubeCount do
        ...
        -- 绘制玻璃外壳
        Renderer.drawGlassShell(nvg, cx + shakeX, cy + bounceY)

        -- 选中光圈
        if game.selectedTube == i then
            nvgBeginPath(nvg)
            nvgEllipse(nvg, cx + shakeX, cy + tube.height / 2 + 8 + bounceY, tube.width / 2, 5)
            nvgFillColor(nvg, nvgRGBA(90, 170, 255, render.selectGlowAlpha))
            nvgFill(nvg)
            nvgBeginPath(nvg)
            nvgEllipse(nvg, cx + shakeX, cy + tube.height / 2 + 8 + bounceY, tube.width / 2 - 6, 3)
...
function GameCanvas:drawAnimSource(nvg, layout, positions)
    ...
    nvgTranslate(nvg, -pivotWX, -pivotWY)
    Renderer.drawGlassShell(nvg, curCX, curCY)
    nvgRestore(nvg)
