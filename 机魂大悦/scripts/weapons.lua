...
local function ApplyBlastDamage(pos, blastRadius, blastDamage, directHitEnemy, owner)
    ...
    if owner ~= "enemy" then
        for _, enemy in ipairs(enemies_) do
            ...
            local enemyCenter = enemy.node.worldPosition + Vector3(0, enemy.hitCenterY or 1.7, 0)
    ...
    -- 爆炸伤害建筑
    DestructibleBuilding.DamageAt(pos, blastDamage, blastRadius)
end
...
function Weapons.UpdateProjectiles(dt)
    ...
    while i <= #projectiles_ do
        ...
        if p.age >= p.life then
                    ...
                    if p.owner ~= "enemy" then
                        ...
                        for _, enemy in ipairs(enemies_) do
                            if enemy.node then
                                local check = hitNode
                                while check do
                                    if check == enemy.node then hitEnemyByRay = enemy; break end
                                    check = check.parent
                                end
                                if hitEnemyByRay then break end
                            end
                        end
                    end

                    if hitEnemyByRay then
                        -- 命中敌人碰撞体：直接造成伤害（而非场景碰撞）
                        hitEnemyCollider = true
                        local def = Weapons.DEFS[p.weaponType]
                        local dmg = p.damage or (def and def.damage or 5)
                        if hitEnemyByRay.hp then
                            hitEnemyByRay.hp = math.max(0, hitEnemyByRay.hp - dmg)
                            print(string.format("[Weapons] Raycast hit enemy collider, dmg=%.1f, hp=%.0f/%d",
                                dmg, hitEnemyByRay.hp, hitEnemyByRay.maxHp or 100))
                        end
                        CreateExplosion(scene, result.position, 0.6, Color(1.0, 0.7, 0.2))
                        if def and def.blastRadius then
                            CreateExplosion(scene, result.position, def.blastRadius, Color(1.0, 0.4, 0.05))
                            local bDmg = (def.blastDamage or 0) * (p.dmgMult or 1.0)
                            ApplyBlastDamage(result.position, def.blastRadius, bDmg, hitEnemyByRay, p.owner)
                            if onExplosionCallback_ then onExplosionCallback_(p.weaponType, result.position) end
                        end
                        if onHitCallback_ then onHitCallback_(p.weaponType, result.position) end
                        RemoveProjectileNode(p)
                        table.remove(projectiles_, i)
                    else
                        hitScene = true
            ...
            if hitEnemyCollider then
                -- 已处理，不再执行后续逻辑（弹丸已移除）
            elseif hitScene then
                -- 命中场景物体（非敌人），创建爆炸效果
                local def = Weapons.DEFS[p.weaponType]
                if def and def.blastRadius then
                    ...
                    -- 非爆炸弹丸命中场景时伤害建筑
                    local def2 = Weapons.DEFS[p.weaponType]
                    local bulletDmg = p.damage or (def2 and def2.damage or 5)
                    DestructibleBuilding.DamageAt(hitPos, bulletDmg, 2.0)
                end
                RemoveProjectileNode(p)
                table.remove(projectiles_, i)
            else
                ...
                if p.owner ~= "enemy" then
                    ...
                    for _, enemy in ipairs(enemies_) do
                        ...
                        local enemyCenter = enemy.node.worldPosition + Vector3(0, enemy.hitCenterY or 1.7, 0)
                        local enemyHitRadius = hitRadius + (enemy.hitRadiusBonus or 0)
                        -- 线段 AB 与目标球体的最近距离检测
