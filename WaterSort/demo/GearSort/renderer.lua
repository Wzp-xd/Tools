...
local GEAR_TEETH      = 12                   -- 齿数（用于相邻槽相位偏移，每槽错开 π/12 = 15°）
...
local IDLE_INTERVAL   = 5.0   -- 闲置转动尝试间隔（秒）
local MAX_STUCK       = 0.0873  -- 卡住时最大转角（弧度 ≈ 5°）
local STUCK_DUR       = 1.4   -- 卡住演出时长（秒）
local FREE_DUR        = 0.8   -- 自由旋转演出时长（秒）
...
function Renderer.RecalcLayout(W, H)
    ...
    local ASSET_RATIO = 343 / 512   -- 素材宽 / 素材高（343×512 px）
    ...
    -- 由 pegH 上限反推 pegW 上限
    local gearSlotFactor = 0.43 * 1.79
    local pegWbyHeight
    if cap > 0 then
    ...
    local pegH_c   = math.floor(gearR_c * 1.79) * cap + math.floor(10 * scale)
    ...
    local slotH = math.floor(gearRBase * 1.79)
...
local function drawGear(ctx, cx, cy, r, color, angle, selected, alpha)
    alpha = alpha or 255
    ...
    local paint = nvgImagePattern(ctx, -r, -r, d, d, 0, img, alpha / 255.0)
    nvgBeginPath(ctx)
    nvgRect(ctx, -r, -r, d, d)
...
function Renderer.Init(ctx)
    ...
    local GEAR_IMAGE_PATHS = {
        red     = "image/gear_red_20260525091020.png",
        blue    = "image/gear_blue_20260525090527.png",
        green   = "image/gear_green_20260525090524.png",
        yellow  = "image/gear_yellow_20260525090535.png",
        purple  = "image/gear_purple_20260525090529.png",
        orange  = "image/gear_orange_20260525090523.png",
        cyan    = "image/gear_cyan_20260525090522.png",
        pink    = "image/gear_pink_20260525090521.png",
        lime    = "image/gear_lime_20260525090522.png",
        brown   = "image/gear_brown_20260525090921.png",
        maroon  = "image/gear_maroon_20260525091139.png",
        navy    = "image/gear_navy_20260525091140.png",
        teal    = "image/gear_teal_20260525091133.png",
        gold    = "image/gear_gold_20260525091146.png",
        violet  = "image/gear_violet_20260525091136.png",
        coral   = "image/gear_coral_20260525091156.png",
        forest  = "image/gear_forest_20260525091137.png",
        rose    = "image/gear_rose_20260525091459.png",
        olive   = "image/gear_olive_20260525091342.png",
        indigo  = "image/gear_indigo_20260525091136.png",
        crimson = "image/gear_crimson_20260525091544.png",
        azure   = "image/gear_azure_20260525091539.png",
        hidden  = "image/gear_hidden_20260525091541.png",
    }
    ...
    pegBaseImg_ = nvgCreateImage(ctx, "image/peg_selected_20260525064430.png", 0)
    if pegBaseImg_ and pegBaseImg_ >= 0 then
        --print("[Renderer] 插板底座图片加载成功 handle=" .. pegBaseImg_)
    else
        pegBaseImg_ = nil
        --print("[Renderer] 插板底座图片加载失败，回退到程序化绘制")
    end

    hubSingleImg_ = nvgCreateImage(ctx, "image/hub_single_slot_20260526063407.png", 0)
    if hubSingleImg_ and hubSingleImg_ >= 0 then
        --print("[Renderer] 临时插板图片加载成功 handle=" .. hubSingleImg_)
    else
        hubSingleImg_ = nil
        --print("[Renderer] 临时插板图片加载失败，回退到程序化绘制")
    end

    pegCapImg_ = nvgCreateImage(ctx, "image/peg_normal_20260525064422.png", 0)
    if pegCapImg_ and pegCapImg_ >= 0 then
        --print("[Renderer] 插板封盖图片加载成功 handle=" .. pegCapImg_)
    else
        pegCapImg_ = nil
        --print("[Renderer] 插板封盖图片加载失败")
    end

    pegBlockerImg_ = nvgCreateImage(ctx, "image/peg_blocker_bar_20260525081847.png", 0)
    if pegBlockerImg_ and pegBlockerImg_ >= 0 then
        --print("[Renderer] 阻挡钢筋图片加载成功 handle=" .. pegBlockerImg_)
    else
        pegBlockerImg_ = nil
        --print("[Renderer] 阻挡钢筋图片加载失败，只进不出插板无阻挡覆盖")
    end

    pegLockedCapImg_ = nvgCreateImage(ctx, "image/edited_peg_locked_cap_nobg_20260525082811.png", 0)
    if pegLockedCapImg_ and pegLockedCapImg_ >= 0 then
        --print("[Renderer] 锁定盖子图片加载成功 handle=" .. pegLockedCapImg_)
    else
        pegLockedCapImg_ = nil
        --print("[Renderer] 锁定盖子图片加载失败，回退到程序化绘制")
    end

    gearHubCapImg_ = nvgCreateImage(ctx, "image/gear_rivet_bearing_20260526071132.png", 0)
    if gearHubCapImg_ and gearHubCapImg_ >= 0 then
        --print("[Renderer] 轴承盖图片加载成功 handle=" .. gearHubCapImg_)
    else
        gearHubCapImg_ = nil
        --print("[Renderer] 轴承盖图片加载失败")
    end

    bgImg_ = nvgCreateImage(ctx, "image/game_bg_vertical_20260525114223.png", 0)
...
-- 获取底部连续同色组：返回 (格数, 实际颜色, 是否被上方异色阻挡)
local function getBottomGroup(pegIdx)
    local peg = GameState.GetPegs()[pegIdx]
    if #peg == 0 then return 0, nil, false end
    local color = GameState.ActualColor(peg[1])
    local n = 0
    for s = 1, #peg do
        if GameState.ActualColor(peg[s]) == color then n = s else break end
    end
    return n, color, (n < #peg)
end

-- 卡住演出的额外转角曲线：0 → MAX → 抖动 → 0
local function stuckCurve(t)
    local M = MAX_STUCK
    if t < 0.25 then
        -- 顺畅推进到最大角
        return M * (1 - math.cos(t / 0.25 * math.pi * 0.5))
    elseif t < 0.72 then
        -- 绕最大角抖动（衰减振幅）
        local u = (t - 0.25) / 0.47
        return M * (1 + math.sin(u * math.pi * 4.5) * 0.38 * (1 - u))
    else
        -- 顺畅退回零点
        local u = (t - 0.72) / 0.28
        return M * math.cos(u * math.pi * 0.5)
    end
end
...
local function drawPegBaseImage(ctx, cx, top, bot, pegW, alpha)
    ...
    -- 按素材宽高比（343:512）以插板高度为基准，避免细长变形
    local ASSET_RATIO = 343 / 512
...
local function drawLockedPeg(ctx, i)
    ...
    for slot = 1, #peg do
        ...
        local angle        = (slot % 2 == 1 and 1 or -1) * (slot - 1) * math.pi / GEAR_TEETH
        drawGear(ctx, cx, slotY(slot, i) - gearLiftY, layout_.gearR, displayColor, angle, false, 90)
    end
    nvgRestore(ctx)
    ...
    local ASSET_RATIO = 343 / 515   -- 素材实际尺寸 343×515
...
-- 动画状态
-- ---------------------------------------------------------------
-- gearAngles_[i] 代表「完成旋转演出」的累计转角（只有全色插满时才累加）
-- 普通插板的齿轮角度固定为 0，额外偏移仅由 stuckAnims_ 提供
local gearAngles_  = {}   -- [pegIdx] -> 持续旋转累计角（仅完成状态）
local spinSpeed_   = 2.0  -- rad/s（完成插板旋转速度）
local time_        = 0
local idleTimers_  = {}   -- [i] 下次尝试转动的倒计时（秒）
local stuckAnims_  = {}   -- [i] { progress, slots, stuck } 卡住/自由旋转演出
...
-- 计算某插板某槽的实际齿轮旋转角
--   slot 奇数（底部开始）正转，偶数反转
--   相邻槽错开半齿角 π/GEAR_TEETH，使齿牙视觉上咬合
--
--   【设计说明】
--   - 普通插板（未完成）：master 固定不动，只有 stuckAnims_ 提供额外偏移
--     - 底部同色组参与演出（stuckAnim），上方异色齿轮角度完全固定（extra=0）
--   - 完成插板（全色插满）：master 持续累加，实现持续旋转
local function getGearAngle(pegIdx, slot)
    local master   = gearAngles_[pegIdx] or 0
    local dir      = (slot % 2 == 1) and 1 or -1
    local phaseOff = (slot - 1) * math.pi / GEAR_TEETH

    -- 卡住 / 自由旋转演出额外偏移
    -- 只作用于 sa.slots 以内的底部同色组；上方异色齿轮 extra=0，保持固定
    local extra = 0
    local sa = stuckAnims_[pegIdx]
    if sa and slot <= sa.slots then
        if sa.stuck then
            extra = stuckCurve(sa.progress)
        else
            -- 自由旋转：sin 弧顺畅完成一段转动（约 270°）
            extra = math.sin(sa.progress * math.pi) * math.pi / 6
        end
    end

    return (master + extra) * dir + phaseOff
end
...
-- Update（每帧调用）
-- ---------------------------------------------------------------
-- 外部触发指定插板的"阻塞抖动"演出（stuck=true）
function Renderer.TriggerStuck(pegIdx)
    if not stuckAnims_[pegIdx] then
        local n = GameState.GetCapacity()
        stuckAnims_[pegIdx] = { progress = 0, slots = n, stuck = true }
    end
end
...
function Renderer.Update(dt, selectedPeg)
    ...
    for i = 1, #pegs do
        ...
        if isCompleted then
            -- ── 完成插板：持续旋转 ──────────────────────────────────────
            gearAngles_[i] = (gearAngles_[i] or 0) + spinSpeed_ * dt

            -- 清除闲置/卡住演出，避免干扰
            stuckAnims_[i] = nil
            idleTimers_[i] = IDLE_INTERVAL
            ...
            -- ── 普通插板：齿轮角度固定，仅演出期间有额外偏移 ────────────
            -- gearAngles_[i] 不再累加，保持静止
            ...
            -- 推进卡住/自由旋转演出
            local sa = stuckAnims_[i]
            if sa then
                local dur = sa.stuck and STUCK_DUR or FREE_DUR
                sa.progress = sa.progress + dt / dur
                if sa.progress >= 1.0 then
                    stuckAnims_[i] = nil
                end
            end

            -- 闲置倒计时（初次随机偏移，避免所有插板同时触发）
            if idleTimers_[i] == nil then
                idleTimers_[i] = math.random() * IDLE_INTERVAL
            end
            idleTimers_[i] = idleTimers_[i] - dt
            if idleTimers_[i] <= 0 and not stuckAnims_[i] then
                idleTimers_[i] = IDLE_INTERVAL + math.random() * 2.0
                -- 只触发底部的闲置演出（判断是否被上方异色阻挡）
                local n, _, blocked = getBottomGroup(i)
                if n > 0 then
                    stuckAnims_[i] = { progress = 0, slots = n, stuck = blocked }
                end
            end
        end
    end
...
function Renderer.Draw(ctx, W, H, selectedPeg, validTargets)
    ...
    for i = 1, #pegs do
        ...
        if not skipGears then
            ...
            for slot = 1, visibleTop do
                -- getGearAngle 统一处理：奇偶方向 + 半齿相位偏移，已完成/普通插板均适用
                local angle = getGearAngle(i, slot)
                ...
                drawGear(ctx, pegX(i), slotY(slot, i), gearR,
                    displayColor, angle, false, alpha)
            end
        end
    end
    ...
    if moveAnim_ then
        ...
        for k = 1, moveAnim_.count do
            ...
            drawGear(ctx, gx, gy, flyGearR, moveAnim_.color, angle, false, 255)
...
function Renderer.DrawFloatingGears(ctx, selectedPeg)
    ...
    for k = 0, count - 1 do
        local slot  = #peg - k
        local angle = getGearAngle(selectedPeg, slot)
        ...
        drawGear(ctx, gx, gy, gearR, displayColor, angle, (k == 0), 255)
        -- 轴承盖跟随浮起齿轮
        if gearHubCapImg_ then
