...
-- "支持作者"按钮状态
local supportBtnHovered_ = false
local supportBtnGlow_ = 0
local supportBtnRect_ = nil   -- {x, y, w, h}
...
local function DrawTitle(ctx, w, h, t, L)
    ...
    nvgText(ctx, w / 2 + 2, titleY + 2, "机魂大悦", nil)

    -- 标题文字
    nvgFillColor(ctx, nvgRGBA(70, 195, 255, 255))
    nvgText(ctx, w / 2, titleY, "机魂大悦", nil)
    ...
    nvgText(ctx, w / 2, subY, "MECH SOUL DELUXE", nil)
end
...
--- 10. "支持作者"按钮（左上角）
local function DrawSupportButton(ctx, w, h, t, L)
    local margin = L.isNarrow and 12 or 20
    local btnW = L.isNarrow and 80 or 100
    local btnH = L.isNarrow and 28 or 34
    local bx = margin
    local by = margin
    local glow = supportBtnGlow_

    supportBtnRect_ = { x = bx, y = by, w = btnW, h = btnH }

    -- 辉光
    if glow > 0.01 then
        local feather = 8 + 8 * glow
        nvgBeginPath(ctx)
        nvgRect(ctx, bx - feather, by - feather, btnW + feather * 2, btnH + feather * 2)
        local haloGrad = nvgBoxGradient(ctx, bx, by, btnW, btnH, 4, feather,
            nvgRGBAf(1.0, 0.7, 0.2, 0.2 * glow),
            nvgRGBAf(0.8, 0.5, 0.1, 0.0))
        nvgFillPaint(ctx, haloGrad)
        nvgFill(ctx)
    end

    -- 背景
    local bgA = math.floor(140 + 60 * glow)
    nvgBeginPath(ctx)
    nvgRoundedRect(ctx, bx, by, btnW, btnH, 4)
    nvgFillColor(ctx, nvgRGBA(
        math.floor(50 + 40 * glow),
        math.floor(30 + 25 * glow),
        math.floor(15 + 15 * glow),
        bgA))
    nvgFill(ctx)

    -- 边框
    local borderA = math.floor(100 + 120 * glow)
    nvgStrokeColor(ctx, nvgRGBA(
        math.floor(200 + 55 * glow),
        math.floor(150 + 50 * glow),
        math.floor(50 + 40 * glow),
        borderA))
    nvgStrokeWidth(ctx, 1.0)
    nvgStroke(ctx)
    ...
    -- 文字
    local fontSize = L.isNarrow and 11 or 13
    nvgFontFace(ctx, "sans")
    nvgFontSize(ctx, fontSize)
    nvgTextAlign(ctx, NVG_ALIGN_CENTER + NVG_ALIGN_MIDDLE)
    local textA = math.floor(200 + 55 * glow)
    nvgFillColor(ctx, nvgRGBA(255, 200, 80, textA))
    nvgText(ctx, bx + btnW / 2, by + btnH / 2, "支持作者", nil)
end
...
function HandleMenuUpdate(eventType, eventData)
    ...
    -- "支持作者"按钮辉光 lerp
    local supportTarget = supportBtnHovered_ and 1.0 or 0.0
    supportBtnGlow_ = supportBtnGlow_ + (supportTarget - supportBtnGlow_) * math.min(1.0, dt * 8.0)
    ...
    hoveredBtn_ = 0
    supportBtnHovered_ = false

    -- 检测"支持作者"按钮 hover
    if supportBtnRect_ then
        local sb = supportBtnRect_
        if mx >= sb.x and mx <= sb.x + sb.w and
           my >= sb.y and my <= sb.y + sb.h then
            supportBtnHovered_ = true
        end
    end

    if not supportBtnHovered_ then
    ...
    -- 触屏支持
    if hoveredBtn_ == 0 and not supportBtnHovered_ and input:GetNumTouches() > 0 then
        ...
        if supportBtnRect_ then
            local sb = supportBtnRect_
            if tx >= sb.x and tx <= sb.x + sb.w and
               ty >= sb.y and ty <= sb.y + sb.h then
                supportBtnHovered_ = true
            end
        end
    ...
    if not clicked and input:GetNumTouches() > 0 then
        local touch = input:GetTouch(0)
        if touch.pressure > 0 and touch.delta.x == 0 and touch.delta.y == 0 then
            clickX = touch.position.x / dpr
            clickY = touch.position.y / dpr
            clicked = true
        end
    end
    ...
    if clicked then
        ...
        if supportBtnRect_ then
            local sb = supportBtnRect_
            if clickX >= sb.x and clickX <= sb.x + sb.w and
               clickY >= sb.y and clickY <= sb.y + sb.h then
                ---@diagnostic disable-next-line: undefined-global
                sdk:ShowRewardVideoAd(function(result)
                    if result.success then
                        print("广告播放成功")
                    else
                        print("广告播放失败" .. result.msg)
                    end
                end)
                return
            end
...
function HandleMenuNanoVGRender(eventType, eventData)
    ...
    DrawSupportButton(menuVg_, w, h, t, L)
